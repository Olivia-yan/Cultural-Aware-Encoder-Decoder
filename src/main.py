...
def print_snapshot_stats(name, snap_list):
    total_triples = 0
    max_triples = 0
    for i, snap in enumerate(snap_list):
        num_triples = len(snap)
        total_triples += num_triples
        max_triples = max(max_triples, num_triples)
        print(f"{name} Snapshot {i}: {num_triples} triples")
    avg_triples = total_triples / len(snap_list) if len(snap_list) > 0 else 0
    print(f"\n{name} Summary:")
    print(f"Total snapshots: {len(snap_list)}")
    print(f"Average triples per snapshot: {avg_triples:.2f}")
    print(f"Max triples in a single snapshot: {max_triples}\n")
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    MAX_TRIPLES = 6000 #tirgn的时候是6000
    train_list = [snap[:MAX_TRIPLES] for snap in train_list]

    # 在加载完 train_list, valid_list, test_list 后调用
    print_snapshot_stats("Train", train_list)
    print_snapshot_stats("Valid", valid_list)
    print_snapshot_stats("Test", test_list)
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                if use_cuda:
                    ...
                    # flops, params = profile(model, inputs=(output[0], static_graph, use_cuda))
